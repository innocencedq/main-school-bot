...
async def settings_keyboard(user):
    ...
    builder.add(InlineKeyboardButton(text="📒 Выключить расширенную кнопку дневника", callback_data='edit_diary')) if diary else None
...
async def advert_kb(curr_id: int, user):
    ...
    builder.add(InlineKeyboardButton(text='Редактировать', callback_data=f'edit_advert-{curr_id}')) if is_admin else None
    builder.add(InlineKeyboardButton(text='⬅️ Назад', callback_data='back'))
