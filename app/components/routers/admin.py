        ...
        if current_index < len(values):
            ...
            try:
                ...
                if current_index < len(values):
                    ...
                    await message.answer('Убедитесь в правильности загруженного расписания!', reply_markup=adm_update_schedule)
            except Exception:
...
async def schedule_update_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    async with async_session() as session:
        for day in values:
            if day_file_id := image_data.get(day):
                stmt = update(Images).where(Images.image_name == day).values(image_id=day_file_id)
                await session.execute(stmt)
